...
export interface Destination {
  ...
  island?: string;
  distance?: number;  // ← untuk nearby feature
...
export function DestinationCard({ destination, onLike, onViewDetails, isLiked, distance, featured = false }: DestinationCardProps) {
  return (
    <Card className={`overflow-hidden hover:shadow-lg transition-shadow duration-200 cursor-pointer active:scale-95 transition-transform ${
      featured ? 'h-full' : ''
    }`}>
      <div className="relative">
        <img
          ...
          className={`w-full object-cover ${
            featured ? 'h-64 sm:h-96' : 'h-40 sm:h-48'
          }`}
        />
        <Button
          variant="ghost"
      ...
      <CardContent className="p-3 sm:p-4" onClick={() => onViewDetails(destination.id)}>
